...
I recommend using a virtual environment, either via `python -m venv` or `pdm venv create`.
...
``` shell
pip install --editable '.[dev,test,batch,jupyter]' # choose what you need
...
The full luigi Task dependency tree in text form as generated by `luigi-deps-tree` given in [`luigi-deps-tree.md`](https://github.com/meliache/yellow-taxis/blob/main/luigi-deps-tree.md).
